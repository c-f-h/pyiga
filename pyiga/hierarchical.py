...
class HMesh:
    ...
    @staticmethod
    def init_from_meshes(meshes, active, deactivated, P=None):
        # assumes each mesh of meshes was generated by calls of TPMesh(kvs)
        out = HMesh(meshes[0])
        out.meshes = meshes
        out.active = active
    ...
    def init_from_KVs(KVs, active, deactivated, P=None):
        ...
        out.meshes = [TPMesh(kvs) for kvs in KVs]
        out.active = active
        out.deactivated = deactivated
        out.P = P
        if not P:
...
class HSpace:
    ...
    def _clear_cache(self):
        ...
        self.__boundary_Hspaces = {bdspec: None for bdspec in self.bdspecs} if self.bdspecs else None
        self.__boundary_mappings = {bdspec: None for bdspec in self.bdspecs} if self.bdspecs else None
        
    @property
    def boundary_HSpaces(self):
        return self.__boundary_Hspaces
    
    @property
    def boundary_mappings(self):
        return self.__boundary_mappings
    
    @staticmethod
    def init_from_meshes(meshes, active_cells, deactivated_cells, active_funcs, deactivated_funcs, P=None, truncate=False, disparity=np.inf, bdspecs=None):
        out = HSpace(KVs[0], truncate=truncate, disparity=disparity, bdspecs=bdspecs)
        out.hmesh = HMesh.init_from_meshes(meshes, active_cells, deactivated_cells, P=P)
    ...
    def _drop_bdspec_tuple_components(self, iterable, bdspec):
        """Remove `bdspec`-components from `iterable` of tuples"""
        if not iterable or not bdspec:
            return set()
        type_iterable = type(iterable)
        out_components = list(range(self.dim))
        out_components.pop(bdspec[0])
        out = np.array(list(iterable.copy()))[:,out_components]
        return type_iterable(tuple(iter) for iter in out)
    ...
    def _dirichlet_indices(self):
        ...
        TPbdspec_indices = {bdspec : [] for bdspec in self.bdspecs} # seperated tensor product indices for each bdspec in bdspecs
        TPbdspec_cells = {bdspec : [] for bdspec in self.bdspecs} # seperated tensor product cells for each bdspec in bdspecs
        for lv in range(self.numlevels):
            aux = set() # collecting bdspecs contributions
            for bdspec in self.bdspecs:
                bdspec_TP_dirichlet_indices = self._compute_single_axis_single_level_dirichlet_indices(lv, bdspec)
                bdspec_TP_dirichlet_cells = self._compute_single_axis_single_level_dirichlet_cells(lv, bdspec)
                TPbdspec_cells[bdspec].append(bdspec_TP_dirichlet_cells)
                TPbdspec_indices[bdspec].append(bdspec_TP_dirichlet_indices)
                aux |= bdspec_TP_dirichlet_indices
            TPbindices.append(aux)

        # Compute separated boundary indices for bdspec in bdspecs
        Hbdspec_active_indices = {bdspec : [] for bdspec in self.bdspecs} # seperated hierarchical indices for each bdspec in bdspecs
        Hbdspec_active_cells = {bdspec : [] for bdspec in self.bdspecs} # seperated hierarchical cells for each bdspec in bdspecs
        Hbdspec_deactivated_indices = {bdspec : [] for bdspec in self.bdspecs} # seperated hierarchical indices for each bdspec in bdspecs
        Hbdspec_deactivated_cells = {bdspec : [] for bdspec in self.bdspecs} # seperated hierarchical cells for each bdspec in bdspecs
        Hbdspecindices_mapping = {bdspec : [] for bdspec in self.bdspecs}
        for bdspec in self.bdspecs:
            for lv in range(self.numlevels):
                aux = self.actfun[lv] & TPbdspec_indices[bdspec][lv]
                Hbdspec_active_indices[bdspec].append(self._drop_bdspec_tuple_components(aux, bdspec))
                Hbdspec_active_cells[bdspec].append(self._drop_bdspec_tuple_components(self.hmesh.active[lv] & TPbdspec_cells[bdspec][lv], bdspec))
                Hbdspec_deactivated_indices[bdspec].append(self._drop_bdspec_tuple_components(self.deactfun[lv] & TPbdspec_indices[bdspec][lv], bdspec))
                Hbdspec_deactivated_cells[bdspec].append(self._drop_bdspec_tuple_components(self.hmesh.deactivated[lv] & TPbdspec_cells[bdspec][lv], bdspec))
                Hbdspecindices_mapping[bdspec].append(aux)
            self.__boundary_mappings[bdspec] = self._boundary_to_original_matrix_indices(Hbdspecindices_mapping[bdspec])
            KVs = tuple(self._drop_bdspec_tuple_components(list(self.hmesh.meshes[lv].kvs for lv in range(self.numlevels)), bdspec))
            self.__boundary_Hspaces[bdspec] = HSpace.init_from_KVs(KVs, Hbdspec_active_cells[bdspec], Hbdspec_deactivated_cells[bdspec], Hbdspec_active_indices[bdspec], Hbdspec_deactivated_indices[bdspec], P=None, truncate=self.truncate, disparity=self.disparity, bdspecs=None)
        #print("active indices: ", Hbdspec_active_indices)
        #print("active cells: ", Hbdspec_active_cells)
        #print("deactivated indices: ", Hbdspec_deactivated_indices)
        #print("deactivated cells: ", Hbdspec_deactivated_cells)
        #print("mappings: ", self.__boundary_mappings)
        print("HSpaces: ", self.__boundary_Hspaces)
